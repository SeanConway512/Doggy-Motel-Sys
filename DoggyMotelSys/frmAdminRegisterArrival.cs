...
{
    public partial class frmAdminRegisterArrival : Form
    {
        frmMainMenu parent;
        Reservations reservations = new Reservations();
        DataSet ds = new DataSet();

        public frmAdminRegisterArrival()
        {
            InitializeComponent();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            conn.Open();
            String strSQL = "UPDATE RESERVATIONS,SET RES_STATUS ='Arrived',WHERE RES_ID = "+ this.textBox1;
            OracleCommand cmd = new OracleCommand(strSQL, conn);
            cmd.ExecuteNonQuery();
            conn.Close();
        }

        private void frmAdminRegisterArrival_Load(object sender, EventArgs e)
        {
